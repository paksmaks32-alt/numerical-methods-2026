import random
import math
import matplotlib.pyplot as plt


# ПУНКТ 1: Генерація матриці та вектора


def generate_data(n=100, exact_x=5):
    A = [[random.uniform(1.0, 10.0) for _ in range(n)] for _ in range(n)]

    # Забезпечення діагонального переважання
    for i in range(n):
        sum_row = sum(abs(A[i][j]) for j in range(n) if j != i)
        A[i][i] = sum_row + random.uniform(1.0, 5.0)

    with open('matrix_A.txt', 'w') as f:
        for row in A:
            f.write(' '.join(map(str, row)) + '\n')

    x_exact_vector = [exact_x for _ in range(n)]
    b = [sum(A[i][j] * x_exact_vector[j] for j in range(n)) for i in range(n)]

    with open('vector_b.txt', 'w') as f:
        for val in b:
            f.write(str(val) + '\n')

    print("Пункт 1 виконано: Файли 'matrix_A.txt' та 'vector_b.txt' успішно створено.\n")



# ПУНКТ 2: Допоміжні математичні функції


def read_matrix(filename):
    with open(filename, 'r') as f:
        return [[float(x) for x in line.split()] for line in f]


def read_vector(filename):
    with open(filename, 'r') as f:
        return [float(line.strip()) for line in f]


def mat_vec_mult(A, x):
    n = len(A)
    return [sum(A[i][j] * x[j] for j in range(n)) for i in range(n)]


def vec_norm(x):
    return max(abs(val) for val in x)


def mat_norm(A):
    return max(sum(abs(val) for val in row) for row in A)

# Ітераційні методи розв'язку СЛАР

def simple_iteration_method(A, b, x0, eps, max_iter=10000):
    n = len(A)
    x = list(x0)
    tau = 1.0 / mat_norm(A)
    iters = 0
    history = []  # Масив для графіка

    while iters < max_iter:
        iters += 1
        Ax = mat_vec_mult(A, x)
        x_new = [x[i] - tau * (Ax[i] - b[i]) for i in range(n)]

        diff = [x_new[i] - x[i] for i in range(n)]
        err = vec_norm(diff)
        history.append(err)  # Зберігаємо похибку

        if err < eps:
            x = list(x_new)
            break
        x = list(x_new)

    return x, iters, history


def jacobi_method(A, b, x0, eps, max_iter=10000):
    n = len(A)
    x = list(x0)
    iters = 0
    history = []  # Масив для графіка

    while iters < max_iter:
        iters += 1
        x_new = [0.0] * n

        for i in range(n):
            s = sum(A[i][j] * x[j] for j in range(n) if j != i)
            x_new[i] = (b[i] - s) / A[i][i]

        diff = [x_new[i] - x[i] for i in range(n)]
        err = vec_norm(diff)
        history.append(err)  # Зберігаємо похибку

        if err < eps:
            x = list(x_new)
            break
        x = list(x_new)

    return x, iters, history


def seidel_method(A, b, x0, eps, max_iter=10000):
    n = len(A)
    x = list(x0)
    iters = 0
    history = []  # Масив для графіка

    while iters < max_iter:
        iters += 1
        x_old = list(x)

        for i in range(n):
            s1 = sum(A[i][j] * x[j] for j in range(i))
            s2 = sum(A[i][j] * x_old[j] for j in range(i + 1, n))
            x[i] = (b[i] - s1 - s2) / A[i][i]

        diff = [x[i] - x_old[i] for i in range(n)]
        err = vec_norm(diff)
        history.append(err)  # Зберігаємо похибку

        if err < eps:
            break

    return x, iters, history

# ПУНКТ 3 та 4: Виконання і пошук розв'язку


if __name__ == "__main__":
    n = 100
    eps = 1e-14

    generate_data(n=n, exact_x=2.5)

    A = read_matrix('matrix_A.txt')
    b = read_vector('vector_b.txt')
    x0 = [1.0 for _ in range(n)]

    print(f"Параметри СЛАР: розмірність {n}x{n}, точність eps = {eps}\n")
    print("-" * 50)

    # Метод простої ітерації (тепер отримуємо 3 змінні, включаючи history)
    x_simple, iters_simple, history_simple = simple_iteration_method(A, b, x0, eps)
    print(f"Метод простої ітерації:")
    print(f"Кількість ітерацій: {iters_simple}")
    print(f"Тестова перевірка (x[0], x[1], x[2]): {x_simple[0]:.4f}, {x_simple[1]:.4f}, {x_simple[2]:.4f}")
    print("-" * 50)

    # Метод Якобі
    x_jacobi, iters_jacobi, history_jacobi = jacobi_method(A, b, x0, eps)
    print(f"Метод Якобі:")
    print(f"Кількість ітерацій: {iters_jacobi}")
    print(f"Тестова перевірка (x[0], x[1], x[2]): {x_jacobi[0]:.4f}, {x_jacobi[1]:.4f}, {x_jacobi[2]:.4f}")
    print("-" * 50)

    # Метод Зейделя
    x_seidel, iters_seidel, history_seidel = seidel_method(A, b, x0, eps)
    print(f"Метод Зейделя:")
    print(f"Кількість ітерацій: {iters_seidel}")
    print(f"Тестова перевірка (x[0], x[1], x[2]): {x_seidel[0]:.4f}, {x_seidel[1]:.4f}, {x_seidel[2]:.4f}")
    print("-" * 50)

    # Побудова графіка

    plt.figure(figsize=(8, 5))

    # Побудова ліній з використанням масивів історії
    plt.plot(history_simple, label='Simple Iteration')
    plt.plot(history_jacobi, label='Jacobi')
    plt.plot(history_seidel, label='Gauss-Seidel')

    plt.yscale('log')
    plt.title('Збіжність методів', fontsize=14)
    plt.xlabel('Iteration', fontsize=12)
    plt.grid(True)
    plt.legend()

    plt.savefig('convergence_plot.png', dpi=300, bbox_inches='tight')
    plt.show()